...
&& GetProp(_fuel_item, ALL_PROPS.fuel) && (recipe[_material_item] == _result_item or !_result_item) //если предмет в ячейке топлива имеет свойство топлива и в ячейке результата такойже предмет который можем приготовить, либо просто пусто
...
&& (recipe[_material_item] == _result_item or !_result_item) && recipe[_material_item]
